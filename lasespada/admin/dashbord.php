...
$frauduleux = $pdo->query("SELECT COUNT(*) FROM sinistres WHERE statut = 'frauduleux'")->fetchColumn();
$douteux = $pdo->query("SELECT COUNT(*) FROM sinistres WHERE statut = 'douteux'")->fetchColumn();
$alertes = $pdo->query("SELECT COUNT(*) FROM alertes WHERE statut = 'en_attente'")->fetchColumn();
    ...
    <div class="main">
      ...
      <div class="content-area">
        ...
        <div class="stat-grid" style="padding:20px">
          ...
          <div class="stat-card">
            <div class="stat-label">Douteux</div>
            ...
            <tbody>
              <?php foreach ($derniers as $s): ?>
                <tr style="border-top:1px solid #e5e7eb">
                  ...
                  <td style="padding:10px">
                    <?= !empty($s["montant_estime"]) ? number_format($s["montant_estime"], 0, ',', ' ') . ' FCFA' : '—' ?>
                  </td>
                  <td style="padding:10px">
                    <?= $s["score_sinistre"] ?>/100
                  </td>
                  <td style="padding:10px">
                    <?= badgeNiveau($s["statut"]) ?>
                  </td>
                  <td style="padding:10px">
                    <a href="detail_sinistre.php?id=<?= $s["id"] ?>"
                      style="color:#2563eb;text-decoration:underline">Voir</a>
                  </td>
                </tr>
              <?php endforeach; ?>
            </tbody>
