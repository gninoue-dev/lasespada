...
$alertes = $pdo->query("
    SELECT a.*, u.nom, u.prenom, s.montant, s.type_sinistre
    FROM alertes a
...
<div class="container">
  ...
  <!-- STATS -->
  <div class="stats">
    <div class="card"><div>Total alertes</div><div class="value"><?= $total ?></div></div>
    <div class="card"><div>Critiques (score ≥ 70)</div><div class="value" style="color:#dc2626"><?= $critiques ?></div></div>
    <div class="card"><div>Non traitées</div><div class="value" style="color:#f59e0b"><?= $total ?></div></div>
  </div>
  ...
  <div class="alert-list">
    ...
    <div class="alert <?= $classe ?>">
      <div class="alert-text">
        <?= $classe === 'critical' ? '🚨' : '⚠️' ?>
        <?= clean($a["type_alerte"]) ?> —
        <?= clean($a["nom"]) ?> <?= clean($a["prenom"]) ?>
        <?php if ($a["montant"]): ?>
            — <?= number_format($a["montant"], 0, ',', ' ') ?> FCFA
        <?php endif; ?>
        <small style="color:#6b7280;margin-left:8px">Score : <?= $a["score_declencheur"] ?></small>
      </div>
